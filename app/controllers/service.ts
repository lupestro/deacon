...
export default class ServiceController extends Controller {
  ...
  _iterate() {
    for (const plate of this.model.plates) {
      const deacon0 = this.model.deacons[0];
      const deacon1 = this.model.deacons[1];
      ...
      const saint = plate.pew.findSaint(plate.seat);
      const deacon = plate.pew.findDeacon(plate.seat);
      if (deacon) {
   ...
   */
  @action start() {
    const firstStage = this.stageNames[0];
  ...
  @action next() {
    ...
    if (index === this.stageNames.length - 1) {
      ...
      const nextStage = this.stageNames[index + 1];
      if (!nextStage) {
        this.terminate;
      } else {
